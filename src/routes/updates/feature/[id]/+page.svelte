...
<script lang="ts">
	...
	import { Layers, Plus, Star } from 'lucide-svelte';
	import { getGraphClient } from '$lib/stores/graph';
	import {
		getFeatureUpdateProfile,
		getFeatureUpdateAssignments,
		assignFeatureUpdate
	...
	async function fetchData(): Promise<void> {
		...
		try {
			...
			const [profileData, assignmentData] = await Promise.all([
				getFeatureUpdateProfile(client, id),
				getFeatureUpdateAssignments(client, id),
	...
	async function refreshAssignments(): Promise<void> {
		const id = page.params.id!;
		const client = getGraphClient();
		const assignmentData = await getFeatureUpdateAssignments(client, id);
		assignments = assignmentData;
	...
	async function handleEditorSave(
		...
		try {
			...
			await assignFeatureUpdate(getGraphClient(), profile.id, updated);
			...
			<div class="panel-raised mb-6">
				<div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
					<div class="flex items-center gap-4">
						...
						>
							<Star size={24} class="text-accent" />
						</div>
						<div>
							<h1 class="text-ink text-2xl font-bold tracking-tight">
								{profile.displayName}
							</h1>
							<p class="text-ink-faint text-sm">Feature Update Profile</p>
						</div>
					</div>
				...
				<div class="mt-4 flex flex-wrap gap-2">
					<Badge variant="info">Feature Update</Badge>
					{#if profile.featureUpdateVersion}
						<Badge variant="outline">{profile.featureUpdateVersion}</Badge>
					{/if}
				...
				<div class="mt-4 grid grid-cols-[repeat(auto-fit,minmax(240px,1fr))] gap-4">
					<div>
						<p class="text-muted text-xs font-medium uppercase">Target Version</p>
						<p class="text-ink text-sm">
							{profile.featureUpdateVersion || 'Not configured'}
						</p>
					</div>
					<div>
						<p class="text-muted text-xs font-medium uppercase">Rollout Start</p>
						<p class="text-ink text-sm">
							{formatDate(profile.rolloutSettings?.offerStartDateTimeInUTC)}
						</p>
					</div>
					<div>
						<p class="text-muted text-xs font-medium uppercase">Rollout End</p>
						<p class="text-ink text-sm">
							{formatDate(profile.rolloutSettings?.offerEndDateTimeInUTC)}
						</p>
					</div>
					{#if profile.rolloutSettings?.offerIntervalInDays}
						<div>
							<p class="text-muted text-xs font-medium uppercase">Offer Interval</p>
							<p class="text-ink text-sm">
								{profile.rolloutSettings.offerIntervalInDays} days
							</p>
						</div>
					{/if}
					<div>
						<p class="text-muted text-xs font-medium uppercase">End of Support</p>
						<p class="text-ink text-sm">
							{formatDate(profile.endOfSupportDate)}
						</p>
					</div>
				</div>
			</div>
